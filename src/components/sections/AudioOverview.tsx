...
export function AudioOverview() {
  ...
  const englishTrack = {
    ...
    url: 'https://res.cloudinary.com/dw3lf8roj/video/upload/v1773202298/The_Kraken_Lounge_s_Radical_South_Texas_Sanctuary_1_vtjdx2.mp4',
  ...
  const handleSpanishPlay = () => {
    playTrack(spanishTrack)
  }

  const isEnglishPlaying = currentTrack?.url === englishTrack.url && isPlaying
              ...
              <div className="relative z-10 space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  >
                    ...
                    <span className="relative z-10">{isEnglishPlaying ? 'Pause' : 'English'}</span>
                    <SpeakerHigh className={`w-5 h-5 transition-all ${isEnglishPlaying ? 'animate-pulse' : ''}`} weight="fill" />
                  </button>
                  ...
                  >
                    <div className="absolute inset-0 bg-gradient-to-t from-white/10 to-transparent opacity-0 group-hover/btn:opacity-100 transition-opacity" />
                    <Globe className="w-5 h-5 group-hover/btn:scale-110 transition-transform" weight="fill" />
                    <span className="relative z-10">{isSpanishPlaying ? 'Pausar' : 'Español'}</span>
                    <SpeakerHigh className={`w-5 h-5 transition-all ${isSpanishPlaying ? 'animate-pulse' : ''}`} weight="fill" />
                  </button>
                </div>
              </div>
            </div>
